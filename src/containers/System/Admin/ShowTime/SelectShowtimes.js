...
class SelectShowtimes extends Component {
    ...
    handleSaveShowtime = async (showtimeData) => {
        ...
        try {
            ...
            console.log("Response from API:", response);  // Log response để kiểm tra
            ...
            if (response && response.status === 'success') {
                ...
                console.log("Error message:", response.message);  // Log chi tiết thông báo lỗi
                if (response.message === 'Thời gian đã bị trùng với suất chiếu khác trong phòng này.') {
                    toast.error('Thời gian đã bị trùng với suất chiếu khác trong phòng này.');
                } else {
                    toast.error('Lỗi khi lưu thông tin suất chiếu.');
                }
            }
        } catch (error) {
            console.error("API call error:", error); // Log chi tiết lỗi từ catch
            if (error.response) {
